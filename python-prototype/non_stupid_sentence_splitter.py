...
LlamaIndex's [SentenceSplitter](https://github.com/run-llama/llama_index/blob/main/llama-index-core/llama_index/core/node_parser/text/sentence.py) includes the length of the metadata as part of the size of the chunk when splitting by sentences. This produces very unintuitive behavior: e.g. when the user specifies a chunk-size of 50 and nodes have metadata of length 40, the resulting split sentences are about 10 tokens long -- as opposed to the specified 50.

This modified SentenceSplitter adds a `include_metadata_in_chunksize` argument that disables the above behavior, ignoring metadata when calculating chunksize (i.e. only including the size of the text datga when calculating chunksize.)
...
class SentenceSplitter(MetadataAwareTextSplitter):
    ...
    include_metadata_in_chunksize: bool = Field(default=True, description="Whether to include metadata length when calculating chunksize. If false, chunks will include roughly `chunk_size` tokens from `text`, ignoring `metadata` completely.")
    ...
    def __init__(
        ...
        include_metadata: bool = True, # whether or not to include the metadata in nodes
        include_metadata_in_chunksize: bool = True,
        ...
        self.include_metadata_in_chunksize = include_metadata_in_chunksize
    ...
    def split_text_metadata_aware(self, text: str, metadata_str: str) -> List[str]:
        ...
        effective_chunk_size = self.chunk_size - metadata_len if self.include_metadata_in_chunksize else self.chunk_size
