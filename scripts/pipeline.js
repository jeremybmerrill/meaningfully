...
let { CustomSentenceSplitter } = await import("../src/main/services/sentenceSplitter")
...
console.log(pipeline.transformations[0].abbreviations);
let nodes = await pipeline.run({ documents });
nodes.forEach((node) => {
  console.log(node["text"]);
});

console.log(pipeline.transformations[0].tokenizer.tokenize(documents[0].text));